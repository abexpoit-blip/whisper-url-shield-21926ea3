...
type DomainStatus = "action_required" | "verifying" | "setting_up" | "ready" | "active" | "offline" | "failed";
...
export const listCustomDomains = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await (supabase as any)
      .from("custom_domains")
      .select("id,domain,status,verification_token,dns_target,is_primary,last_checked_at,verified_at,created_at")
...
export const verifyCustomDomain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const status: DomainStatus = rootOk && wwwOk && txtOk ? "ready" : rootOk || wwwOk || txtOk ? "verifying" : "action_required";
