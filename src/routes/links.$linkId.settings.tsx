...
function LinkSettingsPage() {
  ...
  const [link, setLink] = useState<{ short_code: string; title: string | null; destination_url: string } | null>(null);
  ...
  const load = async () => {
    ...
    setLink({ short_code: linkRow.short_code, title: linkRow.title, destination_url: linkRow.destination_url });
  ...
  useEffect(() => { void load(); /* eslint-disable-next-line */ }, [linkId]);
  ...
  const totalWeight = dests.filter(d => d.is_active && d.weight > 0).reduce((s, d) => s + d.weight, 0);
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 space-y-8">
        ...
        <section className="rounded-2xl border border-border bg-card-gradient p-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Block or allow visitors by country, device, language, or time. Blocked visitors see a safe page.</p>
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <Label>Allowed countries (ISO codes, comma-separated)</Label>
              <Input placeholder="US, GB, BD" value={allowedCountries} onChange={(e) => setAllowedCountries(e.target.value)} />
              <p className="mt-1 text-xs text-muted-foreground">Empty = allow all.</p>
            </div>
            <div>
              <Label>Blocked countries</Label>
              <Input placeholder="RU, CN" value={blockedCountries} onChange={(e) => setBlockedCountries(e.target.value)} />
            </div>
            <div>
              <Label>Allowed languages</Label>
              <Input placeholder="en, bn" value={allowedLangs} onChange={(e) => setAllowedLangs(e.target.value)} />
            </div>
            <div>
              <Label>Blocked languages</Label>
              <Input placeholder="ru, zh" value={blockedLangs} onChange={(e) => setBlockedLangs(e.target.value)} />
            </div>
          </div>
            ...
            {hoursEnabled && (
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <div>
                  ...
                  <Input type="number" min={0} max={23} value={hourStart} onChange={(e) => setHourStart(parseInt(e.target.value) || 0)} />
                </div>
                <div>
                  <Label>End hour (0-23 UTC)</Label>
                  <Input type="number" min={0} max={23} value={hourEnd} onChange={(e) => setHourEnd(parseInt(e.target.value) || 0)} />
                </div>
              </div>
            )}
          </div>
        ...
        <section className="rounded-2xl border border-border bg-card-gradient p-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Add multiple destinations to weight-split traffic. If none are active, the link's default destination is used.
            </p>
          </div>

          <div className="grid gap-3 md:grid-cols-[1fr_180px_100px_auto]">
            <Input placeholder="https://offer-a.com" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} />
            <Input placeholder="Label (optional)" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} />
            <Input type="number" min={1} max={1000} placeholder="Weight" value={newWeight} onChange={(e) => setNewWeight(parseInt(e.target.value) || 1)} />
            <Button onClick={addDest} className="gap-2"><Plus className="h-4 w-4" /> Add</Button>
          </div>

          {dests.length === 0 ? (
            <p className="text-sm text-muted-foreground">No rotator destinations yet — link uses default destination.</p>
          ) : (
            <div className="divide-y divide-border rounded-lg border border-border">
              {dests.map(d => {
                const pct = totalWeight > 0 && d.is_active && d.weight > 0 ? (d.weight / totalWeight * 100).toFixed(1) : "0";
                return (
                  <div key={d.id} className="flex flex-wrap items-center gap-3 p-3">
                    ...
                    <label className="flex items-center gap-1 text-xs">
                      <input type="checkbox" checked={d.is_active} onChange={(e) => void updateDest(d, { is_active: e.target.checked })} />
                      Active
                    </label>
                    <Button size="icon" variant="ghost" onClick={() => removeDest(d.id)} className="text-destructive">
