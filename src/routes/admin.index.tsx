      ...
      <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 md:space-y-8 md:p-8">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0">
            <Badge
              variant="outline"
              className="mb-2 gap-1.5 border-primary/30 bg-primary/5 text-primary"
            >
          ...
          <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0 sm:pb-0">
            <Link to="/admin/payments">
              <Button size="sm" variant="default" className="shrink-0 gap-2">
        ...
        <div className="grid gap-3 grid-cols-2 sm:gap-4 lg:grid-cols-4">
          <KpiCard
            label="Last 24h clicks"
            value={adv?.last24h.total ?? 0}
            sub={`${adv?.last24h.human ?? 0} human · ${adv?.last24h.bot ?? 0} bot`}
            icon={Flame}
          ...
          <KpiCard
            label="Bot ratio (7d)"
            value={`${adv?.last7d.botPct ?? 0}%`}
            sub={`${(adv?.last7d.bot ?? 0).toLocaleString()} of ${(adv?.last7d.total ?? 0).toLocaleString()}`}
            icon={Bot}
            tone="text-rose-400"
            loading={advLoading}
          />
          <KpiCard
            label="Revenue (30d)"
            value={`$${(adv?.revenue.last30d ?? 0).toFixed(2)}`}
            sub={`${Object.keys(adv?.revenue.byPackage ?? {}).length} packages`}
            icon={DollarSign}
            tone="text-emerald-400"
            loading={advLoading}
          />
          <KpiCard
            label="New users (7d / 30d)"
            value={`${adv?.growth.newUsers7d ?? 0} / ${adv?.growth.newUsers30d ?? 0}`}
            sub={`${adv?.growth.activeLinks ?? 0} active · ${adv?.growth.bannedUsers ?? 0} banned`}
            icon={UserPlus}
            tone="text-blue-400"
            loading={advLoading}
          />
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Activity className="h-4 w-4 text-primary" /> Traffic — last 7 days
              </CardTitle>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-rose-400" /> Bot
                </span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <ShieldCheck className="h-4 w-4 text-primary" /> Bot detection (7d)
              </CardTitle>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Globe className="h-4 w-4 text-primary" /> Top countries (7d)
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {(adv?.topCountries?.length ?? 0) === 0 ? (
                ...
                <RankBars
                  items={adv!.topCountries.map((c) => ({ label: c.country, value: c.count }))}
                />
              )}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <ExternalLink className="h-4 w-4 text-primary" /> Top referrers (7d)
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {(adv?.topReferers?.length ?? 0) === 0 ? (
                ...
                <RankBars
                  items={adv!.topReferers.map((r) => ({ label: r.host, value: r.count }))}
                />
              )}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <UserCheck className="h-4 w-4 text-primary" /> Top links (7d)
              </CardTitle>
                  ...
                  <div key={l.id} className="flex items-center justify-between gap-2 text-sm">
                    <div className="min-w-0">
                      ...
                      <p className="truncate font-mono text-xs text-muted-foreground">
                        /{l.short_code} · {l.human}h / {l.bot}b
                      </p>
                    </div>
                    <Badge variant="outline" className="tabular-nums">
                      {l.total}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="lg:col-span-1">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <TrendingUp className="h-4 w-4 text-primary" /> Plan distribution
              </CardTitle>
            ...
            <CardContent className="space-y-3">
              ...
              {Object.entries(data?.planDistribution ?? {})
                ...
                .map(([slug, count]) => {
                  const total =
                    Object.values(data?.planDistribution ?? {}).reduce((a, b) => a + b, 0) || 1;
...
function TrafficChart({
  ...
  series: Array<{ date: string; total: number; bot: number; human: number }>;
}) {
  const w = 560;
  const h = 160;
